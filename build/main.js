...
async function getArray(url, access_token) {
    let allEntries = [];
    let link = url;
    for (let i = 0; i < 20; i++) {
...
async function notifications(access_token) {
    ...
    for (const notif of allNotifications) {
        ...
        // }
        let mentions = notif.status?.mentions?.map((m) => {
        ...
        ${reply ? `<span class="desat">replying to </span>${mentions.map((m) => { return `<a href="${m.url}">@${m.username}</a>`; }).join(' ')}` : ''}
...
async function home(access_token) {
    let timeline = await getArray('https://mastodon.gamedev.place/api/v1/timelines/home', access_token);
...
async function local(access_token) {
    let timeline = await getArray('https://mastodon.gamedev.place/api/v1/timelines/public?local=true', access_token);
    timeline.sort((a, b) => { return scorePost(b) - scorePost(a); });
    console.log(timeline);
    let content_div = document.getElementById('content');
    if (!content_div) {
...
async function tag(access_token, tag) {
    let timeline = await getArray(`https://mastodon.gamedev.place/api/v1/timelines/tag/:${tag}`, access_token);
    let first = true;
    timeline.sort((a, b) => { return scorePost(b) - scorePost(a); });
    console.log(timeline);
    let content_div = document.getElementById('content');
